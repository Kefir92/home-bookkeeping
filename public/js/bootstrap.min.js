...
{	
	...
	HashtagsList.prototype.init = function()
	{
		...
		$(_this.findTag).on('keyup', function(event)
		{
			...
			}, function(data) 
			{
				if(data.action)
				{
					_this.result.text('Найдено: ' + Object.keys(data.hashtags).length);
		...
		{
			if(confirm("Создать хештег '" + _this.findTag.val() + "'?") == true)
			{
	...
	window.Hashtags = function()
	{
		...
		this.findTag 	= $('input[data-action="findTag"]');
		this.btnList 	= $('[data-action="chooseTag"]');
		this.btnAdd 	= $('[data-action="addTag"]');
		this.modalList 	= $('.modalListTags');
		this.hidden		= $('.form-group__tags');
	};
	
	Hashtags.prototype.init = function()
	{
		var _this = this;
		...
		$(_this.findTag).on('keyup', function(event)
		{
			...
			if(!tag.length) {
				_this.btnList.addClass('glyphicon glyphicon-th-list').text('');
				_this.hashtags = [];
				return false;
			}
			...
			}, function(data) 
			{
				if(data.action)
				{
					_this.btnList.removeClass('glyphicon glyphicon-th-list').text(Object.keys(data.hashtags).length);
					_this.hashtags = data.hashtags;
				}
				else
				{
		...
		$(_this.btnList).on('click', function(event)
		{
			if(!Object.keys(_this.hashtags).length)
			{
				return false;
			}
			
			_this.findTag.blur();
			setTimeout(_this.openListModal.bind(_this), 500);
		});
	...
	Hashtags.prototype.openListModal = function()
	{
		var list = _c('div').addClass('list-group');
		
		for (var i in this.hashtags) {
			list.append(_c('a').on('click', this.choose.bind(this, this.hashtags[i]['id'])).addClass('list-group-item').attr('href', 'javascript:void(0);').data('id', this.hashtags[i]['id']).text(this.hashtags[i]['title']).append(_c('span').addClass('badge').text(0)));
		}
		
		this.modalList.find('.modal-content').empty().append(list).end().modal('show');
	};
	
	Hashtags.prototype.choose = function(ID)
	{
		this.choosen.push(ID);
		...
		this.modalList.modal('hide');
	};
	
	Hashtags.prototype.unchoose = function(ID)
	{
		delete this.choosen[this.choosen.indexOf(ID)];
		this.hidden.find('.hashtag_'+ ID).remove();
	};
